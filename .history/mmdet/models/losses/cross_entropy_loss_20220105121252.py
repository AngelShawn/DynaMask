...
class RefineCrossEntropyLoss(nn.Module):
    ...
    def forward(self, stage_instance_preds, stage_detail_preds, semantic_pred, mask_labels, stage_instance_targets, semantic_target):
        loss_mask_set = []
        for idx in range(len(stage_instance_preds)):
            ...
            if idx <= self.start_stage:
                ...
                loss_mask_set.append(loss_mask + self.detail_loss_weight * (loss_detail + loss_detail_))
        ...
        loss_instance = sum([weight * loss for weight, loss in zip(self.stage_instance_loss_weight, loss_mask_set)]) + self.cb_loss_weight * loss_cb
...
class PointRefineCrossEntropyLoss(nn.Module):
    ...
    def forward(self, stage_instance_preds, stage_detail_preds, semantic_pred, stage_instance_targets, semantic_target):
        loss_mask_set = []
        for idx in range(len(stage_instance_preds)):
            instance_pred, instance_target = stage_instance_preds[idx].squeeze(1), stage_instance_targets[idx]
            detail_pred, detail_target = stage_detail_preds[idx].squeeze(1), self.detail_target(instance_target).squeeze(1)          
